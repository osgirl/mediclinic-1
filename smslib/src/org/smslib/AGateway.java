...
public abstract class AGateway
{
	...
	{
		getService().getNotifyQueueManager().getNotifyQueue().add(new GatewayStatusNotification(getGatewayId(), getStatus(), myStatus));
	...
	public void startGateway() throws TimeoutException, GatewayException, IOException, InterruptedException
	{
		...
		this.queueManager = new QueueManager("QueueManager [" + this.gatewayId + "]", getService(), getQueueSchedulingInterval());
		this.restartCount++;
						...
						try
						{
							if (!sendMessage(msg))
							{
								...
								else
								{
									...
									getService().getNotifyQueueManager().getNotifyQueue().add(new OutboundMessageNotification(getGatewayId(), msg));
								}
							}
							else
							{
