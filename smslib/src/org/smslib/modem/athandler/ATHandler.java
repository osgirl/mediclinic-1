...
public class ATHandler extends AATHandler
{
	...
	{
		if (!getGateway().getService().getSettings().DISABLE_CMMS)
		{
	...
	public void readStorageLocations() throws Exception
	{
		...
		try
		{
			...
			else
			{
				...
				getGateway().getService().getLogger().logWarn("CPMS detection failed, proceeding with default storage 'SM'.", null, getGateway().getGatewayId());
			}
		}
		catch (Exception e)
		{
	...
	{
		String command = "AT+CUSD=1,\"" + ussdCommand + "\"\r";

		// get the immediate result code
		String ussdResponse = sendCustomATCommand(command);
		if (!ussdResponse.contains("OK"))
		{
			// TODO some debug output here and maybe some exception
			return null;
		}

		// get the unsolicted result
		ussdResponse = getModemDriver().getResponse();
		/*
		...
		*/
		...
		if (matcher.find()) response = matcher.group(1);
		else response = ussdResponse;
		return response;
	}
