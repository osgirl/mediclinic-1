...
public abstract class AModemDriver
{
	...
	protected void connect() throws TimeoutException, GatewayException, IOException, InterruptedException
	{
		...
		synchronized (getSYNCCommander())
		{
			try
			{
				connectPort();
				setConnected(true);
				setKeepAlive(new KeepAlive("KeepAlive [" + getGateway().getGatewayId() + "]", getGateway().getService(), getGateway().getService().getSettings().SERIAL_KEEPALIVE_INTERVAL * 1000));
				setCnmiEmulationProcessor(new CNMIEmulatorProcessor("CNMIEmulatorProcessor [" + getGateway().getGatewayId() + "]", getGateway().getService(), getGateway().getService().getSettings().CNMI_EMULATOR_INTERVAL * 1000));
			...
			while (isConnected())
			{
				try
				{
					...
					else if (event.event == AsyncEvents.INBOUNDCALL)
					{
						...
						getGateway().getService().getNotifyQueueManager().getNotifyQueue().add(new CallNotification(getGateway().getGatewayId(), getOriginator(response)));
					...
					synchronized (getSYNCInboundReader())
					{
						...
						for (InboundMessage msg : this.msgList)
						{
							switch (msg.getType())
							{
								case INBOUND:
								case STATUSREPORT:
									getGateway().getService().getNotifyQueueManager().getNotifyQueue().add(new InboundMessageNotification(getGateway().getGatewayId(), msg.getType(), msg));
									break;
								default:
									break;
							}
	...
	private class CNMIEmulatorProcessor extends AServiceThread
	{
		...
		public CNMIEmulatorProcessor(String name, Service service, int delay)
		{
			super(name, service, delay, 0, false);
		}
