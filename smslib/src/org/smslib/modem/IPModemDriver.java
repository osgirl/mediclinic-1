...
class IPModemDriver extends AModemDriver
{
	...
	private class Peeker extends Thread
	{
		...
		public void run()
		{
			getGateway().getService().getLogger().logDebug("Peeker started.", null, getGateway().getGatewayId());
			while (true)
			{
				try
				{
			...
			getGateway().getService().getLogger().logDebug("Peeker stopped.", null, getGateway().getGatewayId());
		}
	}
}
