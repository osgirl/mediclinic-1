...
class SerialModemDriver extends AModemDriver implements SerialPortEventListener
{
	...
	{
		if (getGateway().getService().getSettings().SERIAL_NOFLUSH) getGateway().getService().getLogger().logInfo("Comm port flushing is disabled.", null, getGateway().getGatewayId());
		if (getGateway().getService().getSettings().SERIAL_POLLING) getGateway().getService().getLogger().logInfo("Using polled serial port mode.", null, getGateway().getGatewayId());
		try
		{
			...
			if (getGateway().getService().getSettings().SERIAL_POLLING)
			{
				setPortReader(new PortReader("PortReader() [" + getComPort() + "]", getGateway().getService(), getGateway().getService().getSettings().SERIAL_POLLING_INTERVAL));
			}
		}
		catch (Exception e)
		{
