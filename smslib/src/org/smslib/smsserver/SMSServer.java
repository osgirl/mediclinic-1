...
public class SMSServer
{
	...
	void readMessages()
	{
		...
		{
			getService().getLogger().logError("SMSServer: reading messages exception!", e, null);
		}
	}
	...
	void sendMessages()
	{
		...
		if (foundOutboundGateway)
		{
			...
			{
				getService().getLogger().logError("SMSServer: sending messages exception!", e, null);
			}
			if (getProperties().getProperty("settings.send_mode", "sync").equalsIgnoreCase(("sync")))
			{
				...
				for (OutboundMessage msg : msgList)
				{
					...
					catch (Exception e)
					{
						...
						{
							getService().getLogger().logError("SMSServer: sending messages exception!", e1, null);
						}
					}
				}
			}
			...
			{
				getService().getLogger().logInfo("SMSServer: sending asynchronously... [" + msgList.size() + "]", null, null);
				for (OutboundMessage msg : msgList)
				{
					if (!getService().queueMessage(msg))
					{
						try
						{
							for (Interface<? extends Object> inf : getInfList())
	...
	public static void main(String[] args)
	{
		...
		catch (Exception e)
		{
			...
			{
				app.srv.getLogger().logError("SMSServer error while shutting down: ", e1, null);
			}
		}
	}
}
