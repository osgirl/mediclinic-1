...
public class Service
{
	...
	public boolean sendMessage(OutboundMessage msg) throws TimeoutException, GatewayException, IOException, InterruptedException
	{
		...
		if (gateway != null)
		{
			...
			catch (IOException e)
			{
				...
				//System.out.println("Service.sendMessage()"+e.getMessage());
				gateway.setStatus(GatewayStatuses.RESTART);
				msg.setMessageStatus(MessageStatuses.FAILED);
				msg.setFailureCause(FailureCauses.GATEWAY_FAILURE);
				return false;
	...
	public boolean queueMessageAt(OutboundMessage msg, Date at)
	{
		if (getServiceStatus() != ServiceStatus.STARTED) return false;
		List<String> recipients = expandGroup(msg.getRecipient());
		if (recipients.size() == 0) return getQueueManager().queueDelayedMessage(msg, at);
		for (String to : recipients)
		{
			OutboundMessage newMessage = new OutboundMessage();
			msg.copyTo(newMessage);
			newMessage.setRecipient(to);
			getQueueManager().queueDelayedMessage(msg, at);
		}
		return true;
	}
	...
	public boolean queueMessageAt(OutboundMessage msg, long delayMillis)
	{
		if (getServiceStatus() != ServiceStatus.STARTED) return false;
		List<String> recipients = expandGroup(msg.getRecipient());
		if (recipients.size() == 0) return getQueueManager().queueDelayedMessage(msg, delayMillis);
		for (String to : recipients)
		{
			OutboundMessage newMessage = new OutboundMessage();
			msg.copyTo(newMessage);
			newMessage.setRecipient(to);
			getQueueManager().queueDelayedMessage(msg, delayMillis);
		}
		return true;
	}
	...
	{
		public WatchDog(String name, Service service, int delay)
		{
		...
		public void process() throws Exception
		{
			...
			for (AGateway gateway : getGateways())
			{
				...
				{
					getLogger().logWarn("Gateway: " + gateway.getGatewayId() + " restarting.", null, null);
					try
					{
						gateway.stopGateway();
						gateway.setStatus(GatewayStatuses.RESTART);
					...
					{
						gateway.setStatus(GatewayStatuses.RESTART);
						getLogger().logWarn("Error while shutting down Gateway: " + gateway.getGatewayId(), e, null);
					}
					try
					{
						gateway.startGateway();
					...
					{
						gateway.setStatus(GatewayStatuses.RESTART);
						getLogger().logError("Error while starting Gateway: " + gateway.getGatewayId(), e, null);
					}
				}
			}
		}
